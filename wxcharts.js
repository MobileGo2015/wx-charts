...
var config = {
    ...
    colors: ['#f4f4f4', '#f7a35c', '#434348', '#90ed7d', '#f15c80', '#8085e9'],
...
function drawPieDataPoints(series, opts, config, context) {
    ...
    if (opts.type === 'ring') {
        ...
        //画等分的中间的白线
        if (opts.drawWhiteLine){
          drawWhiteStrokeLine(series, opts, config, context);
        }
    }
...
function drawWhiteStrokeLine(series, opts, config, context){
  var process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
  var pieOption = opts.extra.pie || {};
  series = getPieDataPoints(series, process);
  var centerPosition = {
  ...
  var _start_ = (pieOption.offsetAngle || 0) * Math.PI / 180;
  var count = opts.whiteLineCount;
  var _proportion_ = 1 / count*process;
  ...
  for (var i = 0; i <count;i++){
      ...
      context.moveTo(centerPosition.x, centerPosition.y);
      context.arc(centerPosition.x, centerPosition.y, radius, _start_ + i * _proportion_ * 2 * Math.PI, _start_+2*_proportion_*Math.PI+i*_proportion_*2*Math.PI);
      context.closePath();
      context.fill();
      context.stroke();
  }
